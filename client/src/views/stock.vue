...
async function getData() {
  data = await axios.get(`http://localhost:3000/api/stock`).then((res) => {
    return res.data
  })
...
function openNew() {
  this.modalData = {}
  this.modalData.title = 'Добавить груз'
...
<template>
  ...
  <main class="pt-2 px-3">
    <button class="btn btn-primary" @click="openNew()">Зарегистрировать новый груз</button>
    <br />
    <br />
    <table class="table table-striped">
      ...
      <tbody>
        <tr v-for="item in data" :key="item.id">
          ...
          <td scope="col">{{ item.destination }}</td>
          <td scope="col">{{ item.arrival_date }}</td>
